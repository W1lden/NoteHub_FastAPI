...
from notes.db.models import Category, User
...
async def notes_list_no_slash(request: Request, session: AsyncSession = Depends(get_async_session)):
    return await notes_list(request, session)
...
async def notes_create_form(request: Request, session: AsyncSession = Depends(get_async_session)):
    ...
    return templates.TemplateResponse("notes/create.html", {"request": request, "user": user, "categories": categories})
...
async def notes_detail(note_id: int, request: Request, session: AsyncSession = Depends(get_async_session)):
    ...
    return templates.TemplateResponse("notes/detail.html", {"request": request, "user": user, "note": note, "is_owner": is_owner})
...
async def notes_edit_form(note_id: int, request: Request, session: AsyncSession = Depends(get_async_session)):
    ...
    return templates.TemplateResponse("notes/edit.html", {"request": request, "user": user, "note": note, "categories": categories, "selected_ids": selected_ids})
