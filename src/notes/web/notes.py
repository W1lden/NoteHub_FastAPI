...
async def notes_list(
    ...
    return templates.TemplateResponse(
        "notes/list.html", {"request": request, "notes": notes}
    )


@router.get("/new", response_class=HTMLResponse)
async def notes_create_form(
    request: Request, session: AsyncSession = Depends(get_async_session)
):
    templates = request.app.state.templates
    user = await get_current_user(request, session)
    ...
    return templates.TemplateResponse(
        "notes/create.html", {"request": request, "categories": categories}
    )
...
async def notes_create(
    ...
    note = await note_crud.create_with_categories(
        obj_in=obj_in, category_ids=category_ids, session=session
    )
    return RedirectResponse(
        url=f"/notes/{note.id}", status_code=status.HTTP_303_SEE_OTHER
    )
...
async def notes_detail(
    ...
    return templates.TemplateResponse(
        "notes/detail.html",
        {"request": request, "note": note, "is_owner": is_owner},
    )


@router.get("/{note_id}/edit", response_class=HTMLResponse)
async def notes_edit_form(
    note_id: int,
    request: Request,
    session: AsyncSession = Depends(get_async_session),
):
...
async def notes_edit(
    ...
    await note_crud.update_with_categories(
        db_obj=note, obj_in=update_data, session=session
    )
    return RedirectResponse(
        url=f"/notes/{note_id}", status_code=status.HTTP_303_SEE_OTHER
    )
